<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="p-4">
      ...
      <!-- Video Groups -->
      <div v-else>
        <div v-for="group in videoGroups" :key="group.id" class="border-b border-gray-400 pb-2 mb-4">
          <!-- Group Title -->
          <h2 class="text-base font-semibold text-black mb-3">{{ group.title }}</h2>

          <!-- Video Items -->
          <div class="space-y-1">
            <NuxtLink v-for="video in group.sub_category" :key="video.id"
              :to="{ path: `/video/play/${video.id}`, query: { title: video.title } }"
              class="flex items-center gap-3 py-2 hover:bg-gray-50 pl-4">
              <!-- Indonesian Flag -->
              ...
              <!-- Video Title -->
              <span class="text-sm text-gray-700 flex-1">{{ video.title }}</span>
            </NuxtLink>
          </div>
        </div>
      </div>
